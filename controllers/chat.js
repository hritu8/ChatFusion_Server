...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length > 100) {
    return next(new ErrorHandler("Chat can't have more than 100 members", 400));
  }
  await chat.save();
...
const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  if (files.length > 5)
    return next(new ErrorHandler("Atmost 5 attachments are allowed", 400));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (!chat.groupChat && !chat.members.includes(req.user.toString())) {
    return next(
      new ErrorHandler("You are not allowed to delete the chat", 404)
    );
  }

  // here we have to delete all messages as well as attachments
  // or files from cloudinary
